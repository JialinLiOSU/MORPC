...
def Indicator1_7b(data_dic):
    ...
    #df_oh1['count'] = 1    
    # regional 
    df1_year = df_oh1.groupby(['year','technology'],as_index = False)['capacity_(mw)'].sum()
    pivot_df = df1_year.pivot(index='year', columns='technology', values='capacity_(mw)')
    region = pivot_df.fillna(0)
    region.reset_index(inplace=True)
    # county
    grouped_county = df_oh1.groupby(['county','technology','year'],as_index = False)['capacity_(mw)'].sum()
    pivot_df_county = grouped_county.pivot_table(index=['county','year'], columns='technology', values='capacity_(mw)')
...
if __name__ == '__main__':
    ...
    region,county = Indicator1_7b(data_dict)
