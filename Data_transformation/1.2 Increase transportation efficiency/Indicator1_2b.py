#Increase Transportation Efficiency :alternative transport mode
...
def indicator1_2b(dfs):
    ...
    final_region = region[['carpool','pub_transic','bicycle','walk','work_home']].div(region.tot, axis=0)
    final_region  = final_region.reset_index()
    #county
    county = selectdf.drop(['drove'],axis=1)
    county[['carpool','pub_transic','bicycle','walk','work_home']] = county[['carpool','pub_transic','bicycle','walk','work_home']].div(county.tot, axis=0)
    final_county = county.drop('tot', axis=1)
    print('Indicator 1.2b finished process for year '+ str(dfs[0]['created_at'].year))
...
if __name__== "__main__":
    ...
    region,county = indicator1_2b(data_dict)'''
