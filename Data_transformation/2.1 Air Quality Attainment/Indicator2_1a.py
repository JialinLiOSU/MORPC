...
import pickle
import datetime
#ozone attainment:year by year ingestion
def Indicator2_1a(data_dic):
    '''
        input:[
            data_dict: {
    ...
    df_oh = df1.loc[(df1['id'].isin(['39101','39091', '39159', '39097', '39047', '39041','39117', '39049', \
                                    '39129','39141','39083', '39089','39045', '39127', '39073'])) & (df1['first_max_value'] > 0.07), \
                                    ['id','first_max_value','date_local']].copy()
    
    #transform value to category
    if len(df_oh)!= 0:
        df_oh.loc[(df_oh['first_max_value'] >0.07) & (df_oh['first_max_value'] <= 0.085), 'aqi_category'] = "unhealthy_for_sensitive_groups"
        df_oh.loc[(df_oh['first_max_value'] >0.085) & (df_oh['first_max_value'] <= 0.105), 'aqi_category'] = "unhealthy"
        df_oh.loc[df_oh['first_max_value'] >0.105 , 'aqi_category'] = "very_unhealthy"
    ...
    print('Indicator2.1a transform success for year ' + str(data_dic[0]['created_at'].year))
...
if __name__ == '__main__':
    #with open("output_list.pkl", "rb") as fp:
        #b = pickle.load(fp)
    data_dict = test('ozone2000.xlsx')
    region,county = Indicator2_1a(data_dict)'''
